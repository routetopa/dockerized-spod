...
final class OW_Database
{
    ...
    private function prepareMysql()
    {
        if ( $this->connection->getAttribute(PDO::ATTR_DRIVER_NAME) == 'mysql' )
        {
            ...
            if ( (intval($verArray[0]) == 5 && intval($verArray[1]) >=7 && intval($verArray[2]) >= 9) || intval($verArray[0]) == 8 )
            {
                $this->connection->exec(' SET SESSION sql_mode = ""; SET default_storage_engine=InnoDB; ');
            }
        }
    }
